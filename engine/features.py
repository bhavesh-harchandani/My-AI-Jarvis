...
import pyttsx3  # Direct yahan se bolne ke liye taaki circular import na ho

# 🔑 APNI REAL GEMINI API KEY YAHAN PAR PASTE KAREIN
genai.configure(api_key="AIzaSyADsSz5wNgsG87QhiDflItyt7dvaob9fys")

def geminai(query):
    try:
        model = genai.GenerativeModel('gemini-pro')
        prompt = f"You are Jarvis, a smart AI assistant. Answer this question in a short, clean, and interactive way (maximum 2-3 sentences) so it's easy to speak: {query}"
        
        response = model.generate_content(prompt)
        answer = response.text
        clean_answer = answer.replace("*", "").strip()
        
        print(f"Jarvis AI Response: {clean_answer}")
        
        # Direct yahan engine initialize karke bolwa rahe hain taaki crash na ho
        ...
        # Agar API key galat hui ya internet nahi chala toh terminal par error dikhega
